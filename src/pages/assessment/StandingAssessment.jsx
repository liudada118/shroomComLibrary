...
export default function StandingAssessment() {
  ...
  // ─── 通过 useChannelData 订阅 foot1 通道数据 ───
  useChannelData('foot1', useCallback((channel, matrix, meta) => {
    // 收到真实设备数据时，自动切换到 connected 状态
    if (deviceStatus !== 'connected' && !simIntervalRef.current) {
  ...
  }, [deviceStatus, handleSerialData]));

  // ─── 连接真实设备（通过 DeviceManager）───
  ...
  useEffect(() => {
    // 保留旧接口兼容
    try { serialService.setFilterThreshold(filterThreshold); } catch(_) {}
  }, [filterThreshold]);
      ...
      <header className="assessment-header">
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          <div className="hidden sm:flex items-center gap-2">
            <DevicePanel compact assessmentType="standing" />
            ...
            {deviceStatus === 'connected' && (
              <button onClick={handleDisconnect} className="text-xs font-medium px-2 py-1 rounded" style={{ color: C.red, background: 'rgba(220,38,38,0.08)' }}>断开</button>
            )}
          </div>
          <span className="text-sm font-semibold hidden md:inline" style={{ color: 'var(--text-primary)' }}>{patientInfo?.name || '---'}</span>
