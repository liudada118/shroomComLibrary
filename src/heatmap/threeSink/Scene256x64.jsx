...
const buildFallbackData = (width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT) => {
  const data = new Array(height);
  for (let r = 0; r < height; r++) {
    data[r] = new Array(width);
    for (let c = 0; c < width; c++) {
      const dx = (c - width / 2) / (width / 2);
      const dy = (r - height / 2) / (height / 2);
...
const normalizeTo2D = (input, width, height) => {
  if (!Array.isArray(input)) return buildFallbackData(width, height);
...
export function Scene256x64({
  ...
  const textureSizeRef = useRef({
    width: DEFAULT_WIDTH,
    height: DEFAULT_HEIGHT,
    scale: DEFAULT_UPSCALE,
    max: 1
  });
  ...
  useEffect(() => {
    ...
    // Create texture
    const baseData = sourceDataRef.current || buildFallbackData(DEFAULT_WIDTH, DEFAULT_HEIGHT);
    const rawData = normalizeTo2D(baseData, DEFAULT_WIDTH, DEFAULT_HEIGHT);
    const max_val = typeof sourceMaxRef.current === 'number' ? sourceMaxRef.current : 255;
    const scale = Math.max(1, Math.round(upscale));
    const rawWidth = rawData[0]?.length || DEFAULT_WIDTH;
    const rawHeight = rawData.length || DEFAULT_HEIGHT;
