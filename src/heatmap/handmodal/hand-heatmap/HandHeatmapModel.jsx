...
function normalizeHandType(handType) {
  if (handType === undefined || handType === null) return 'left'
  const value = String(handType).toLowerCase()
  if (value === 'right' || value === 'r' || value === '2') return 'right'
  return 'left'
}
...
function arrX2Y(arr) {
  ...
  if (n % 1 !== 0) {
    throw new Error('Input array length must be a perfect square.')
  }
...
function handRVideo1470506(arr) {
  ...
  handPointArr.forEach((a, index) => {
    ...
    if (![1, 2, 16, 17, 31, 32, 46, 47, 61, 62].includes(index)) {
      newZeroArr[(a[0] + 1) * 32 + 31 - a[1]] = handArr[index]
    }
  })

  return flipVertical(newZeroArr, 32, 32)
}
...
function handSkinChange(res) {
  ...
  const resCopy = [...res]
  for (let i = 4 * 15; i < 5 * 15; i++) {
    resCopy[i] = resCopy[i] / 3
  }

  const res1 = []
  for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 15; j++) {
      res1.push(resCopy[i * 15 + 14 - j])
    }
  }
  ...
  for (let i = 0; i < 4; i++) {
    for (let j = 0; j < 15; j++) {
      res1.push(resCopy[75 + 12 + i * 15 + 14 - j])
    }
  }
...
export function HandHeatmapModel({
  data,
  handType = 'left',
  isRecording = false,
  pressureValue = 0
  ...
  const [internalHandType, setInternalHandType] = useState('left')

  const resolvedHandType = handType ?? internalHandType
  const isLeftHand = useMemo(() => normalizeHandType(resolvedHandType) === 'left', [resolvedHandType])
  const resolvedData = useMemo(() => {
    const source = data ?? internalData
    if (Array.isArray(source)) return source
    if (source && Array.isArray(source.arr)) return source.arr
    return null
  }, [data, internalData])

  useEffect(() => {
    if (!heatmapRef.current) {
      heatmapRef.current = new HeatmapCanvas(30, 30, 1, 1, 'hand', HEATMAP_OPTIONS)
      setHeatmapCanvas(heatmapRef.current.canvas)
    }
  }, [])

  useEffect(() => {
    if (typeof window === 'undefined') return
    const handler = (nextHandType, nextData) => {
      const arr = Array.isArray(nextData) ? nextData : nextData?.arr
      if (!Array.isArray(arr) || arr.length !== 256) {
        console.warn('[HandHeatmapModel] updateHandHeatmap expects length 256 array')
        return
      }
      setInternalHandType(nextHandType ?? 'left')
      setInternalData(arr)
    }
    window.updateHandHeatmap = handler
    return () => {
      if (window.updateHandHeatmap === handler) {
        delete window.updateHandHeatmap
      }
    }
  }, [])

  useEffect(() => {
    if (!heatmapRef.current || !resolvedData || resolvedData.length !== 256) return
    const mapped = isLeftHand
      ? handSkinChange(handL(resolvedData))
      : handRVideo1470506(resolvedData)
    heatmapRef.current.changeHeatmap(mapped, 1, 1, 0)
    setHeatmapVersion((v) => v + 1)
  }, [resolvedData, isLeftHand])
...
export default HandHeatmapModel
